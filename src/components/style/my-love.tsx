...
export const Heart = styled(
  ({
    pulsate,
    ...props
  }: HeartProps & React.ImgHTMLAttributes<HTMLImageElement>) => (
    <img {...props} />
  )
)<HeartProps>`
  position: absolute;
...
export const HeartBeat: React.FC = () => {
  ...
  const handlePasswordSubmit = (password: string) => {
    const correctPassword = "hamilton";
  ...
  return (
    <>
      <Heart
        ...
        onClick={handleClick}
      />
      {showPasswordInput && <PasswordInput onSubmit={handlePasswordSubmit} />}
      <p style={{ marginTop: "300px" }}>hint: Our fav play</p>
    </>
  );
};
